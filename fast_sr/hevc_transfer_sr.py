...
@function
def hevc_transfer_sr(sr_result=None,N_frames=None,hevc_info=None,params=None,*args,**kwargs):
    varargin = hevc_transfer_sr.varargin
    nargin = hevc_transfer_sr.nargin

    if logical_not(exist('params','var')):
        params=[]
# hevc_transfer_sr.m:5
    
    if logical_not(isfield(params,'transfer_thresh')):
        params.transfer_thresh = copy(10)
# hevc_transfer_sr.m:9
    
    if logical_not(isfield(params,'deblock')):
        params.deblock = copy(1)
# hevc_transfer_sr.m:13
    
    if logical_not(isfield(params,'QP')):
        params.QP = copy(47)
# hevc_transfer_sr.m:17
    
    b_debug=0
# hevc_transfer_sr.m:20
    # ------------------------------------------------------------------------
# Parse Parameters
# ------------------------------------------------------------------------
    img_width=size(sr_result,2)
# hevc_transfer_sr.m:24
    img_height=size(sr_result,1)
# hevc_transfer_sr.m:25
    # ------------------------------------------------------------------------
# Reconstruct low-resolution image
# ------------------------------------------------------------------------
    recon_together=combine_dumped_info(hevc_info.intra_recon,hevc_info.inter_mc,hevc_info.res_all,hevc_info.inter_mask)
...
# hevc_transfer_sr.m:90
                    img_h_transfer[f_idx][arange((y_h + 1),(y_h + dot(2,h))),arange((x_h + 1),(x_h + dot(2,w)))]=ref_h_patch + res_h_patch
...
# hevc_transfer_sr.m:103
                    img_h_transfer[f_idx][arange((y_h + 1),(y_h + dot(2,h))),arange((x_h + 1),(x_h + dot(2,w)))]=imresize(inter_patch,2,'bicubic')
...
# hevc_transfer_sr.m:118
                    img_h_transfer[f_idx][arange((y_h + 1),(y_h + dot(2,h))),arange((x_h + 1),(x_h + dot(2,w)))]=imresize(intra_patch,2,'bicubic')
