...
@function
def parse_dumped_coeff_multiple_poc(file_path=None,*args,**kwargs):
    varargin = parse_dumped_coeff_multiple_poc.varargin
    nargin = parse_dumped_coeff_multiple_poc.nargin
...
# parse_dumped_coeff_multiple_poc.m:12
    res_frame_all[1]=struct('x',cellarray([[]]),'y',cellarray([[]]),'w',cellarray([[]]),'residual',cellarray([[]]))
# parse_dumped_coeff_multiple_poc.m:13
    res_frame_all[2]=struct('x',cellarray([[]]),'y',cellarray([[]]),'w',cellarray([[]]),'residual',cellarray([[]]))
# parse_dumped_coeff_multiple_poc.m:14
    res_frame_all[3]=struct('x',cellarray([[]]),'y',cellarray([[]]),'w',cellarray([[]]),'residual',cellarray([[]]))
# parse_dumped_coeff_multiple_poc.m:15
    PU_frame_all=struct('intra',cellarray([[]]),'x',cellarray([[]]),'y',cellarray([[]]),'w',cellarray([[]]),'h',cellarray([[]]),'mv_x',cellarray([[]]),'mv_y',cellarray([[]]),'t',cellarray([[]]),'t_r',cellarray([[]]),'luma_mode',cellarray([[]]))
...
# parse_dumped_coeff_multiple_poc.m:47
            if all(levels == 0):
                ...
                assert_(all(coeffs == 0) and all(residuals == 0),'A TU with all zero scalings has non zero coeffs & residuals')
            tid=- 1
...
# parse_dumped_coeff_multiple_poc.m:60
                #                 x, y, text, intra);
                # Initialize a TU
                res_struct=struct('x',cellarray([x]),'y',cellarray([y]),'w',cellarray([[]]),'residual',cellarray([[]]))
...
# parse_dumped_coeff_multiple_poc.m:71
                    PU_frame=struct('intra',cellarray([1]),'x',cellarray([prop_map('x')]),'y',cellarray([prop_map('y')]),'w',cellarray([prop_map('w')]),'h',cellarray([prop_map('h')]),'mv_x',cellarray([[]]),'mv_y',cellarray([[]]),'t',cellarray([prop_map('t')]),'t_r',cellarray([[]]),'luma_mode',cellarray([prop_map('luma_mode')]))
...
# parse_dumped_coeff_multiple_poc.m:79
                        PU_frame=struct('intra',cellarray([0]),'x',cellarray([prop_map('x')]),'y',cellarray([prop_map('y')]),'w',cellarray([prop_map('w')]),'h',cellarray([prop_map('h')]),'mv_x',cellarray([prop_map('mv_x')]),'mv_y',cellarray([prop_map('mv_y')]),'t',cellarray([prop_map('t')]),'t_r',cellarray([prop_map('t_r')]),'luma_mode',cellarray([[]]))
...
@function
def parse_prop(line=None,*args,**kwargs):
    varargin = parse_prop.varargin
    nargin = parse_prop.nargin
