...
@function
def parse_all_saved_info(dec_info=None,*args,**kwargs):
    varargin = parse_all_saved_info.varargin
    nargin = parse_all_saved_info.nargin
...
# parse_all_saved_info.m:16
    if db_var == 1:
        ...
        save(fullfile(cd,'temp_data','db_parse_all_saved_info.mat'),'dec_info','PU_all','res_luma_all','img_width','img_height','N_frames')
        load(fullfile(cd,'temp_data','db_parse_all_saved_info.mat'),'dec_info','PU_all','res_luma_all','img_width','img_height','N_frames')
...
# ------------------------------------------------------------------------
    other_info=struct('PU',cellarray([PU_all]),'TU',cellarray([res_luma_all]),'mv_x',cellarray([mv_x_map]),'mv_y',cellarray([mv_y_map]))
