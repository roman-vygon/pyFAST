...
# start_fast_sr.m:5
    if logical_not(exist(result_path,'dir')):
        mkdir(result_path)
    
    temp_path=fullfile(cd,'..','temp_data')
# start_fast_sr.m:10
    if logical_not(exist(temp_path,'dir')):
        mkdir(temp_path)
...
# ------------------------------------------------------------------------
    save_file=fullfile(cd,'..','temp_data',sprintf('%s_info.mat',seq_name))
# start_fast_sr.m:47
...
# start_fast_sr.m:69
        Y_low_res=load_Y_of_yuv(dec_info.enc_info.yuv_recon_name,dec_info.enc_info.img_width,dec_info.enc_info.img_height,num_frames)
# start_fast_sr.m:73
        hevc_info=struct('intra_recon',cellarray([intra_recon]),'inter_mc',cellarray([inter_mc]),'res_all',cellarray([res_all]),'inter_mask',cellarray([inter_mask]),'other_info',cellarray([other_info]))
# start_fast_sr.m:77
